...
Cổng quản trị tập trung cho **nhân sự nền tảng** (platform personnel). Khác với buyer/seller (xác thực qua PostgreSQL), nhân sự nền tảng sẽ xác thực qua **Domain Controller (LDAP) + MFA** federate vào Keycloak `ecommerce-realm` (Phase 4). Hiện test bằng tài khoản Keycloak-local (`admin1`).
...
| DB | `admin_portal` (audit/cache); đọc thêm `ecommerce` và `shopfood` để hiển thị vận hành |
| Nguồn danh tính | Keycloak-local (Phase 3) -> Domain Controller LDAP + MFA (Phase 4) |
...
- Duyệt KYC: gán/thu hồi role `kyc-verified` (review tài liệu CCCD/passport vẫn ở ShopPay `/kyc/admin`).
...
│   ├── kyc/                              # duyệt KYC (gán/thu hồi kyc-verified)
│   ├── audit/                            # audit log
│   ├── denied/                           # trang chặn
...
- LDAP user federation (Windows Server AD) + group->role mapper cho nhân sự nền tảng.
