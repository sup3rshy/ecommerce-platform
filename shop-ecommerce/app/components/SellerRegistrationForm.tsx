...
type SellerRegistrationFormProps = {
  ...
  // Tái dùng cho cả seller (ShopSell) và food-seller (ShopFood). Mặc định là seller.
  endpoint?: string;
  fieldLabel?: string;
  placeholder?: string;
  submitLabel?: string;
...
export default function SellerRegistrationForm({
  ...
  fieldLabel = "Tên gian hàng",
  placeholder = "Ví dụ: Cửa hàng An Nhiên",
  submitLabel = "Hoàn tất đăng ký bán hàng",
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(payload?.error ?? "Không thể gửi đăng ký.");
        return;
      }
