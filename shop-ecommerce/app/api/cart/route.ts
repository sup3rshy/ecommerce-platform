...
export async function POST(req: Request) {
  ...
  const productRows = await db
    .select({ id: products.id, stock: products.stock, storeId: products.storeId, status: products.status })
    .from(products)
  ...
  if (productRows[0].status !== "active") {
    return NextResponse.json({ error: "Sản phẩm không còn được bán." }, { status: 409 });
  }
