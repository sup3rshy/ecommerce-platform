...
export async function PATCH(_request: Request, { params }: ApproveSellerRequestParams) {
  ...
  const requestRows = await db
    .select({
      ...
      kind: sellerUpgradeRequests.kind,
  ...
  // kind quyết định role cấp: food-seller (ShopFood) hay seller (ShopSell).
  const isFoodSeller = requestRow.kind === "food-seller";
  const grantedRole = isFoodSeller ? "food-seller" : "seller";

  try {
    await assignRealmRoleToUser(requestRow.userId, grantedRole);
  } catch (error) {
    console.error(`Failed to assign ${grantedRole} role:`, error);
    return NextResponse.json({ error: `Không thể cấp quyền ${grantedRole} trên Keycloak.` }, { status: 502 });
  }

  await db.transaction(async (tx) => {
    // Chỉ seller mới có record stores (catalog ShopEcommerce). food-seller quản lý
    // menu ở ShopFood (DB riêng) nên không tạo stores ở đây.
    if (!isFoodSeller) {
  ...
  return NextResponse.json({
    success: true,
    message: `Đã phê duyệt yêu cầu và cấp quyền ${grantedRole} thành công.`,
  });
}
