...
export default async function Home({ searchParams }: PageProps) {
  ...
  const selectedCategory = resolvedSearchParams.category?.trim() || null;
  ...
  if (selectedCategory) {
    filters.push(eq(products.category, selectedCategory));
  }

  const whereClause = filters.length > 0 ? and(...filters) : undefined;

  const [allProducts, countResult, categoryRows] = await Promise.all([
    db
      .select()
      .from(products)
      .where(whereClause)
  ...
  return (
    <main className="w-full space-y-6">
      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-[280px_1fr]">
        <ProductSearchFilters categories={categories} selectedCategory={selectedCategory ?? undefined} />

        <div className="min-w-0">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-2xl font-bold text-slate-900">Danh sách sản phẩm</h2>
            <p className="text-sm text-slate-500">Tìm thấy {totalCount} sản phẩm</p>
          </div>
